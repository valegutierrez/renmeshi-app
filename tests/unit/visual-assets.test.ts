...
describe('canonical visual assets', () => {
  it('uses the supplied seigaiha asset and currentColor', () => {
    ...
    expect(css).toContain('color: var(--outline-variant)')
  })
  ...
  it('uses the square logo as an On Primary mask in the footer', () => {
    ...
    expect(css).toContain('background-color: var(--on-primary)')
    expect(logo).toContain('fill:currentColor')
  })

  it('defines all required semantic tokens for every supported mode', () => {
    expect(tokens.match(/--rm-primary:/g)).toHaveLength(4)
    expect(tokens.match(/--rm-on-primary:/g)).toHaveLength(4)
    expect(tokens.match(/--rm-outline-variant:/g)).toHaveLength(4)
    expect(css).toContain('--primary: var(--rm-primary)')
    expect(css).toContain('--on-primary: var(--rm-on-primary)')
    expect(css).toContain('--outline-variant: var(--rm-outline-variant)')
  })
})